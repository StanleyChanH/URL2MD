# 基于ReaderLMv2的html信息获取

## 项目模块
1. 传入URL获取真实的html，用一切手段获取真实的html
    - 至少包括以下模块：
      - 处理重定向
      - 设置合适的请求头
      - 管理Cookies和会话
      - 应对动态加载内容
      - 处理AJAX请求异步加载
2. 基于html用ReaderLMv2解析正文内容：
    1. ReaderLMv2调用方式，本地OLLAMA调用，模型名称readerlmv2，请使用openaiSDK调用
    2. 官方教程操作：
       1. 预先清理 HTML，删除脚本、样式、注释，以减少输入的噪声和长度
        ```python
        import re
        ...
        ```
3. 通过Fast API将整个流程（传入url获取markdown或者json）封装成API接口，供前端调用。